...
class SDFSP(QWidget):
    ...
    def __init__(self, debug: bool = False, parent: Optional[QWidget] = None):
        ...
        self.field_widgets: Dict[str, Dict[str, Any]] = (
            {}
        )  # Stocke les widgets par nom de field
    ...
    def create_field_widget(self, field: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if field.get("field_description"):
            collapse_btn = QPushButton(
                "↓" if not field.get("collapsed", False) else "→"
            )
    ...
    def load_fields(self, fields: List[Dict[str, Any]]):
        ...
        for field_data in fields:
            if self.debug:
                logging.debug(
                    f"Loading field: {field_data.get('field_name', 'Unknown')}"
                )
...
class SDFSPv2(QWidget):
    ...
    def load_stack_data(self, data: Dict[str, Any]):
        ...
        if self.debug:
            logging.info(
                f"{self.__class__.__name__}::{inspect.currentframe().f_code.co_name}: Loading stack data"
            )
        ...
        title_label.setToolTip(
            f"{data.get('stacked_title', 'Untitled')}{shortcut_text}"
        )
