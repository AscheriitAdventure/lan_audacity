...
class DynFormDialog(QDialog):
    ...
    def load_form(self, form_data: Dict[str, Any]):
        ...
        for field in form_data.get('fields', []):
            ...
            if self.debug:
                logging.info(
                    f"{self.__class__.__name__}::{inspect.currentframe().f_code.co_name}: Field added at row {current_row}")
    ...
    def load_form_input(self, input_config: Dict[str, Any], row: int) -> Tuple[QWidget, int]:
        ...
        if self.debug:
            logging.info(
                f"{self.__class__.__name__}::{inspect.currentframe().f_code.co_name}: Creating input of type {input_config.get('type')} at row {row}")
    ...
    def load_form_field(self, field: Dict[str, Any], row: int) -> int:
        ...
        if self.debug:
            logging.info(
                f"{self.__class__.__name__}::{inspect.currentframe().f_code.co_name}: Loading field at row {row}")
