...
def createWebAddress(
    ...
    if not any([ipv4, mask_ipv4, ipv4_public, cidr, ipv6_local, ipv6_global]):
        logging.debug(f"{self.__class__.__name__}::{inspect.currentframe().f_code.co_name}: Au moins un des paramètres doit être fourni.")
    ...
    if cidr and (not ipv4 or not mask_ipv4):
        try:
            ...
            logging.error(f"{self.__class__.__name__}::{inspect.currentframe().f_code.co_name}: Erreur lors de l'analyse du CIDR: {e}")
