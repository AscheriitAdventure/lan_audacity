...
class LanAudacity:
    ...
    def setSavePath(self, new_path: str) -> None:
        if os.path.exists(new_path):
            ...
            if os.path.isdir(path_part1) and os.path.exists(
                f"{path_part1}/{self.refFile}"
            ):
                logging.warning(
                    f"The project {self.projectName} already exists in {new_path}."
                )
                self.load_project(new_path)
            else:
                self.savePath = new_path
                logging.info(
                    f"The save path of {self.projectName} is set to {new_path}."
                )
    ...
    def load_project(self, path: str) -> None:
        ...
        if os.path.exists(path_part1) and os.path.exists(
            f"{path_part1}/{self.refFile}"
        ):
